...
import Diadoc.Api.Proto.Docflow.DocflowApiV4Protos;
...
import static Diadoc.Api.Proto.Docflow.DocflowApiV4Protos.*;
...
public class DocflowClient {
    ...
    public GetDocflowEventsResponseV4 getDocflowEventsV4(String boxId, GetDocflowEventsRequest getDocflowEventsRequest) throws DiadocSdkException {
        if (Tools.isNullOrEmpty(boxId)) {
            throw new IllegalArgumentException("boxId");
        }
        try {
            var request = RequestBuilder.post(
                    new URIBuilder(diadocHttpClient.getBaseUrl())
                            .setPath("/V4/GetDocflowEvents")
                            .addParameter("boxId", boxId)
                            .build())
                    .setEntity(new ByteArrayEntity(getDocflowEventsRequest.toByteArray()));
            return GetDocflowEventsResponseV4.parseFrom(diadocHttpClient.performRequest(request));
        } catch (URISyntaxException | IOException e) {
            throw new DiadocSdkException(e);
        }
    }
    ...
    public SearchDocflowsResponseV4 searchDocflowsV4(String boxId, SearchDocflowsRequest docflowsRequest) throws IOException, DiadocSdkException {
        if (Tools.isNullOrEmpty(boxId)) {
            throw new IllegalArgumentException("boxId");
        }
        try {
            var request = RequestBuilder.post(
                    new URIBuilder(diadocHttpClient.getBaseUrl())
                            .setPath("/V4/SearchDocflows")
                            .addParameter("boxId", boxId)
                            .build())
                    .setEntity(new ByteArrayEntity(docflowsRequest.toByteArray()));
            return SearchDocflowsResponseV4.parseFrom(diadocHttpClient.performRequest(request));
        } catch (URISyntaxException | IOException e) {
            throw new DiadocSdkException(e);
        }
    }
    ...
    public GetDocflowsByPacketIdResponseV4 getDocflowsByPacketIdV4(String boxId, GetDocflowsByPacketIdRequest docflowsByPacketIdRequest) throws IOException, DiadocSdkException {
        if (Tools.isNullOrEmpty(boxId)) {
            throw new IllegalArgumentException("boxId");
        }
        try {
            var request = RequestBuilder.post(
                    new URIBuilder(diadocHttpClient.getBaseUrl())
                            .setPath("/V4/GetDocflowsByPacketId")
                            .addParameter("boxId", boxId)
                            .build())
                    .setEntity(new ByteArrayEntity(docflowsByPacketIdRequest.toByteArray()));
            return GetDocflowsByPacketIdResponseV4.parseFrom(diadocHttpClient.performRequest(request));
        } catch (URISyntaxException | IOException e) {
            throw new DiadocSdkException(e);
        }
    }
